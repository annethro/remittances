...
b1 <- as_tibble(read.dta("Burkina Faso/f1w.dta")) #member count, ages
b2 <- as_tibble(read.dta("Burkina Faso/f2w.dta")) #rooms, construction
b3 <- as_tibble(read.dta("Burkina Faso/f3w.dta")) #items
...
b1a <- b1 %>%
  select(men, id) #List of everyone who *currently* lives in the house.
...
bur1c <- bur1c %>%
  ...
  rename(house = men, 
         own_house = s21, own_land = s311, wall = s23, rm_cook = s24, rm_count = s25, electric = s26, water = s27, radio = s316, tv = s317, fridge = s318, auto = s3117, moto = s3118, tele_mobile = s3113, tele_land = s3114)
...
bur1c$ppl_room <- bur1c$pres_size/bur1c$rm_count # number of people per room. Number of people per sleeping room is preferred; this is second choice when sleeping room data are not available. Produces 19 NAs as expected

bur1c$ppl_room[bur1c$ppl_room == 44] <- NA # remove not-credible extreme outlier: 1 room with 44 people in it
...
bur1c$wall[bur1c$wall == "Other"] <- NA

# Tell R which variables (columns) to use in imputation for which variables (rows)

predMat <- matrix(rep(0, ncol(bur1c)^2), ncol = ncol(bur1c), nrow = ncol(bur1c))

fromthis <- ifelse(colnames(bur1c) %in% c("own_house", "radio", "tv", "fridge", "tele_mobile", "tele_land", "auto", "moto", "wall", "electric"), 0, 1)
predMat[colnames(bur1c) %in% c("own_land", "rm_cook", "water", "ppl_room"), ] <- fromthis
...
bur1d <- complete (mice (bur1c, method = "pmm", predictorMatrix = predMat, seed = 17000731)) # Seed chosen randomly on June 14 2024. PMM means predictive mean matching; DHS protocol uses mean assignment for the DHS wealth index. Complete returns the data set (saved as dat1) with missing values populated with predicted values.

bur1d <- droplevels(bur1d) # Dropping levels to improve behavior of factors

### Back to recoding wealth data ###

bur1d$own_land <- case_when(
  bur1d$own_land == "yes" ~ 1,
  bur1d$own_land == "No" ~ 0
)

bur1d$own_house <- case_when(
  bur1d$own_house == "Owned" ~ 1,
  .default = 0
)

bur1d$wall <- case_when(
  bur1d$wall == "Bricks/Stones" ~ "cement",
  bur1d$wall == "Flaw" ~ "natural", #this is straw, cross-referencing the French
  bur1d$wall == "Wood" ~ "natural",
  bur1d$wall == "Mud" ~ "natural",
  bur1d$wall == "Other" ~ "other"
...
bur1d$wall <- as_factor(bur1d$wall)
  
setDT(bur1d)[, c(paste0("wall_",levels(bur1d$wall)), "wall") := 
...
bur1d$water <- case_when(
  bur1d$water == "Well in the coumpound" ~ "private",
  bur1d$water == "Internal faucet/tap" ~ "private",
  bur1d$water == "External faucet/tape" ~ "private", #intuition: like Bolivia, sometimes the pipe comes right outside the door, and this is different than a shared public well
  bur1d$water == "Water truck" ~ "public",
  bur1d$water == "Public pump" ~ "public",
  bur1d$water == "Well outside the coumpound" ~ "public", #based on reports of NGOs, these are very likely to be public wells
  bur1d$water == "River or stream" ~ "public"
) #natural materials are one category

bur1d$water <- as_factor(bur1d$water)

setDT(bur1d)[, c(paste0("water_",levels(bur1d$water)), "water") := 
...
bur1d$fridge <- case_when(
  bur1d$fridge == "yes" ~ 1,
  bur1d$fridge == "No" ~ 0
)

bur1d$tele <- case_when(
      bur1d$tele_mobile == "yes" | bur1d$tele_land == "yes" ~ 1,
      .default = 0
    ) # accprding to standard DHS practice, landline and mobile both count

bur1d$auto <- case_when(
  bur1d$auto == "yes" ~ 1,
  bur1d$auto == "No" ~ 0
)

bur1d$moto <- case_when(
  bur1d$moto == "yes" ~ 1,
  bur1d$moto == "No" ~ 0
)

bur1d$rm_cook <- case_when(
  bur1d$rm_cook == "yes" ~ 1,
  bur1d$rm_cook == "no" ~ 0
)

bur1d$electric <- case_when(
  bur1d$electric == "yes" ~ 1,
  bur1d$electric == "no" ~ 0
)
...
bur1d$wealth_index <- pca1$x[ ,1] # household-level score on first principal component. Explains 32% of the variance in wealth.
...
# NEED TO ADD A ROW COUNT FOR HOUSEHOLD MIGRANTS EVEN IF DIDN'T REMIT -- ROWS

bur1e <- left_join(bur1d, baggs, by = "house")

##### Merge with location data #####

bur1e <- bur1e %>%
  select(house, prov, vill, wealth_index, pres_size, remit) %>%
  rename(loc_meso = prov, loc_micro = vill)

# loc_meso = prov, loc_micro = vill,
