...
##### Download data #####
# Download from https://microdata.worldbank.org/index.php/catalog/402
...
n5a <- n5 %>%
  select(HHNo, CensusUnit, Town, Migrantin, Howlong, Howmanytimes, Sendfoodgoods) %>%
...
n6a <- n6 %>%
  select(HHNo, Censusunit, Where, Sendmoney, Foodgoods) %>% # Some houses (12) didn't receive money or goods in last 12, so need to exclude those.
...
n5a <- droplevels(n5a) # Dropping levels to improve behavior of factors

n5a <- n5a %>%
  rename(hh_money = Howmanytimes, hh_goods = Sendfoodgoods) # curr variables are for current HH members who are elsewhere; nothh variables are for non-HH members who are elsewhere


## Recode remittance data

n5a$hh_money <- case_when(
  n5a$hh_money > 0 ~ 1,
  n5a$hh_money == 0 ~ 0)

n5a$hh_goods <- case_when(
  n5a$hh_goods == "Yes" ~ 1,
  n5a$hh_goods == "No" ~ 0
)

n5a$hh_remit <- case_when(
  n5a$hh_goods == 1 | n5a$hh_money == 1 ~ 1,
  n5a$hh_goods == 0 & n5a$hh_money == 0 ~ 0 # True zeroes are only those that have zeroes for both columns
)

n5a <- n5a %>%
  filter(!is.na(hh_remit) | !is.na(Howlong) | !is.na(Migrantin), na.rm = TRUE) # remove rows where we have no location, no length of time in the place, or no remittance -- these appear to be errors.

### Received a remittance from non-household member ###

n6a$nonhh_remit <- case_when(
  n6a$Sendmoney == "Yes" | n6a$Foodgoods == "Yes" ~ 1,
  n6a$Sendmoney == "No" & n6a$Foodgoods == "No" ~ 0
)

n6b <- n6a %>%
  group_by(censusunit, hhno) %>% # need to group first to use keep in mutate
  filter(!is.na(nonhh_remit) | !is.na(Where), na.rm = TRUE) %>% # remove rows where there was no location for non-hh migrant or no remittance coming in -- these appear to be added in error
  select(-Sendmoney, -Foodgoods) # Remove unused columns

n5a <- n5a %>%
  rename(censusunit = CensusUnit)

n56 <- bind_rows(n5a, n6b) # Combine rows from households with HH members elsewhere and HHs with social-network members elsewhere
...
n56 <- n56 %>%
  mutate(hh_loc = if_else(Migrantin == "Urban area within Nigeria" | Migrantin == "Rural area within Nigeria", "national", "international"), nonhh_loc = if_else(Where == "Urban area within Nigeria" | Where == "Rural area within Nigeria", "national", "international"), .keep = "unused")

### Code how many months a household migrant has been in current location ###
# For Nigeria, these are in years (including fractions of a year) -- multiply by 12 for comparability to other countries

n56 <- n56 %>%
  rowwise() %>%
  mutate(hh_months = Howlong * 12, .keep = "unused") # Mostly converts into clean integers; some remaining decimals

### Combine columns and remove unused ones ###

n56$remit <- case_when(
  n56$hh_remit == 1 & is.na(n56$nonhh_remit) ~ 1,
  n56$nonhh_remit == 1 & is.na(n56$hh_remit) ~ 1,
  n56$nonhh_remit == 0 & n56$hh_remit == 0 ~ 0
) # This is cleanest, to avoid overwriting any data... the below (unite) can be less clean but checked for potential overwriting before doing this with n56[!is.na(n56$hh_loc) & !is.na(n56$nonhh_loc),]

n56 <- n56 %>% unite("migrant_loc", hh_loc, nonhh_loc, na.rm = TRUE)
...
nga1e <- left_join(nga1d, n56, by = c("censusunit", "hhno"))
...
nga1e <- nga1e %>%
  select(house, state, censusunit, country, wealth_index, hh_size, migrant_num, any_remit, migrant_hh, migrant_loc, hh_months, remit) %>% # Simplify and reduce participant identifiability
...
loc <- mutate(loc, landmark = NULL, notes = NULL) # Remove notes about how she got lat/long for each

nga1f <- left_join(nga1e, loc, by = c("loc_micro" = "location")) # Merge province location with household data

nga1f <- nga1f %>%
  filter(loc_micro != "jangachi") %>% # This tract is missing data for migrants (interview questions weren't asked); omit
