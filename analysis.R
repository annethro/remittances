...
sub_15$autocorrelation_pos <- -sub_15$autocorrelation

sub_15 <- sub_15 %>%
  mutate(annual_frequency_s = scale(annual_frequency), severity_s = scale(severity_pos), autocorrelation_s = scale(autocorrelation_pos), avg_area_km_s = scale(avg_area_km))
...
ests_mod15$parameters <- c("Intercept", "Severity", "annual_frequency", "autocorrelation", "Spatial extent", "Wealth", "Household size", "Migrant number", "Dist. to pop. center", "Mean NDVI")


ests_mod15 <- ests_mod15 %>%
  mutate(parameters = factor(parameters, levels = c("Mean NDVI", "Dist. to pop. center", "Migrant number", "Household size", "Wealth", "Spatial extent", "autocorrelation", "annual_frequency", "Severity", "Intercept")))
